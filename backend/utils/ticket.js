...
function line(ch = '-', width = 32) {
  return ch.repeat(width);
}

function money(n) {
  return (Number(n).toFixed(2) + ' Bs').padStart(11, ' ');
}
...
function buildTicket(s) {
  ...
  lines.push(line());
  lines.push('Consumos:');
  if (Array.isArray(s.consumos) && s.consumos.length) {
    ...
    lines.push('  (sin consumos)');
  }
  lines.push(line());
  lines.push(`Subtotal consumos: ${money(s.subtotal_consumos_bs)}`);
  if (s.descuento_bs && s.descuento_bs > 0) {
    lines.push(`Descuento:        -${money(s.descuento_bs)}`);
  }
